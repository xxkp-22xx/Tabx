...
contract TabX {
    ...
    // Expense split events for off-chain indexing
    event SplitCreated(
    ...
    // Event-only split functions (off-chain indexing)
    function createSplit(
        ...
        require(debtors.length == amounts.length, "Mismatched arrays");
        emit SplitCreated(groupId, msg.sender, name, debtors, amounts, block.timestamp);
    }
    ...
    function editSplit(
        ...
        require(debtors.length == amounts.length, "Mismatched arrays");
        emit SplitEdited(groupId, msg.sender, originalIdx, name, debtors, amounts, block.timestamp);
    }

    function deleteSplit(uint256 groupId, uint256 originalIdx)
        external
        onlyRegistered
        groupExists(groupId)
    {
        emit SplitDeleted(groupId, msg.sender, originalIdx, block.timestamp);
    }
